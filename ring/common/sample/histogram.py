...
class ScaleHistogram:
    ...
    def bucket_index_newFeature(self, target_values):
        assert len(target_values) > 0
        scale = max(np.abs(target_values))
        scale_bin = int(math.log(scale + 1.0, self._base))
        return scale_bin

    def bucket_index_combine(self, target_values):
        assert len(target_values) > 0
        scale_bin = self.bucket_index(target_values)
        scale_bin_newFeature = self.bucket_index_newFeature(target_values)
        scale_bin_combine = (scale_bin, scale_bin_newFeature)
        return scale_bin_combine

    def add(self, target_values):
        if len(target_values) > 0:
            bucket = self.bucket_index_combine(target_values)
    ...
    def count(self, target):
        if len(target) > 0:
            return self.bin_counts[self.bucket_index_combine(target)]
        else:

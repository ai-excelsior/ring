...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):  # n_top: c*ln(L_q)
        ...
        # use the M_top Q to calculate Q_K
        Q_reduce = Q[
...
class AutoCorrelation(nn.Module):
    ...
    def time_delay_agg_inference(self, values, corr):
        ...
        if torch.cuda.is_available():
            ...
            init_index = (
                torch.arange(length).unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(batch, head, channel, 1)
            )
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        if self.training:
            ...
            V = self.time_delay_agg_inference(values.permute(0, 2, 3, 1).contiguous(), corr).permute(
                0, 3, 1, 2
            )

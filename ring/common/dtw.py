...
def pairwise_distances(x: torch.Tensor, y: torch.Tensor = None) -> torch.Tensor:
    ...
    Args:
        x (torch.Tensor): [*, N]
        y (torch.Tensor, optional): [*, M]. Defaults to None.
    ...
    if y is None:
        y = x
    assert x.dim() == y.dim(), "x,y should have same dimensions"

    dims = x.shape[:-1]

    x = x.reshape((*dims, -1, 1))
    y = y.reshape((*dims, 1, -1))

    x_norm = x ** 2
    y_norm = y ** 2

    dist = x_norm + y_norm - 2.0 * torch.matmul(x, y)
...
@jit(nopython=True)
def max_hessian_product(p, z, gamma):
    return (p * z - p * np.sum(p * z)) / gamma


@jit(nopython=True)
def min_hessian_product(p, z, gamma):
    return -max_hessian_product(p, z, gamma)
...
def dtw_hessian_prod(theta, Z, Q, E, gamma):
    ...
    for i in range(1, m + 1):
        for j in range(1, n + 1):
            ...
            Q_dot[i, j] = min_hessian_product(Q[i, j], v, gamma)

...
class dagmm(BaseAnormal):
    ...
    def forward(self, x: Dict[str, torch.Tensor], mode=None, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        batchsize = enc_output.shape[0]
        # reconstruction error
        rec_cosine = F.cosine_similarity(
            x["encoder_cont"].reshape(batchsize, -1), dec.reshape(batchsize, -1), dim=1
        )
        rec_euclidean = (x["encoder_cont"].reshape(batchsize, -1) - dec.reshape(batchsize, -1)).norm(
            2, dim=1
        ) / torch.clamp(x["encoder_cont"].reshape(batchsize, -1).norm(2, dim=1), min=1e-10)
        ...
        if mode != "predict":
            self.compute_gmm_params(z, gamma, batchsize)
            if self.training:
    ...
    def compute_gmm_params(self, z: torch.tensor = None, gamma: torch.tensor = None, batchsize: int = None):
        # shape = k_clusters
        sum_gamma = torch.sum(gamma, dim=0)
        # shape = k_clusters
        self.phi = sum_gamma / batchsize
    ...
    def compute_energy(self, z: torch.tensor = None, size_average=True, phi=None, cov=None, mu=None):
        ...
        eps = 1e-10
        cov_inverse = [
            torch.inverse((self.cov[i] + torch.eye(self.hidden_size + 2) * eps).unsqueeze(0))
            for i in range(self.k_clusters)
        ]
        det_cov = [
            torch.linalg.det((self.cov[i] + torch.eye(self.hidden_size + 2) * eps) * (2 * np.pi)).unsqueeze(0)
            for i in range(self.k_clusters)
        ]
        cov_diag = sum(
            [
                torch.sum(1 / (self.cov[i] + torch.eye(self.hidden_size + 2) * eps).diag())
                for i in range(self.k_clusters)
            ]
        )
        ...
        # k_clusters
        det_cov = torch.cat(det_cov, dim=0)
        # batch_size * k_clusters, torch.sum() to reduce the dimension back
        ...
        sample_energy = -max_val.squeeze() - torch.log(
            torch.sum(
                self.phi.unsqueeze(0) * exp_term / (torch.sqrt(det_cov) + eps).unsqueeze(0),
                dim=1,
            )
            + eps
        )
